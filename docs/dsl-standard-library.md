...
state付きDSPを`fn note`のcall treeで使うとVoiceごとのstate、`fn effect`のcall treeで使うとPlugin全体のglobal stateになります。同じsource位置・種類・sample rateでhot reloadした場合はstateを引き継ぎます。

## Worker Evaluation Compatibility

`note` call treeでは`voice` storageだけを宣言・参照できます。`global` storageは`filter` call treeだけで利用でき、旧`note` storage domainはコンパイルエラーです。このため各Voiceのpersistent scalar、RingBuf、stateful DSPは他Voiceと共有されず、固定affinityのblock workerで安全に並列評価されます。

MIDI event間のsegmentごとにworkerが`evaluate_note_block()`を実行し、worker-local mixをmain threadでreduceした後、`filter`の`evaluate_filter_block()`を1回実行します。`filter`内のGlobal scalar、RingBuf、DSPはplugin instanceで単一のstateを持ち、sample順のtransactional commitを維持します。
...
Reverbをpost-mix処理として使う場合は`fn effect`内で左右それぞれに異なるcall siteを用意すると、左右の内部stateも独立します。
